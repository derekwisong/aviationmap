...
class Api:
    ...
    def set_color(self, station, color, show=False):
        result = self._post(
            f"/station/{station}/color", json={"color": color, "show": show})
        return result.ok

    def set_colors(self,
                   colors: typing.Dict[str, typing.List[int]],
                   show=False):
        return self._post("/stations/color",
                          json={"colors": colors, "show": show}).ok
